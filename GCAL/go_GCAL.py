...
if __name__ == '__main__':
    ...
    batch_size = 20
    if args.sent_len:
        h = GCAL(platform, MAX_SENTENCE_LENGTH=int(args.sent_len), alter_params='sent_len')
    elif args.comt_len:
        h = GCAL(platform, MAX_COMS_LENGTH=int(args.comt_len), alter_params='comt_len')
    elif args.comt_cout:
        h = GCAL(platform, MAX_COMS_COUNT=int(args.comt_cout), alter_params='comt_cout')
    elif args.sent_cout:
        h = GCAL(platform, MAX_SENTENCE_COUNT=int(args.sent_cout), alter_params='sent_cout')
    elif args.user_comt:
        h = GCAL(platform, USER_COMS=int(args.user_comt), alter_params='user_comt')
    elif args.learning:
        h = GCAL(platform, lr= round(float(args.learning)/10000,6), alter_params='lr')
    elif args.bat_size:
        h = GCAL(platform, alter_params='batch_size')
        batch_size = int(args.bat_size)
    else:
        h = GCAL(platform, alter_params='null')
